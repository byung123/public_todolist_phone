...
// TodoAll 복사한 것 밑에 if절만 추가해줘서 완료를 눌렀을 때 같은 형식의 내용이 나오게끔
function TodoToday(props) {
    ...
    useEffect(() => {
        ...
        for(let todo of todolistAll.todolist) {
            ...
            const months = parse(now.getMonth() + 1);
            const day = parse(now.getDate());
            const today = years + "-" + months + "-" + day
            const specifyDay = todo.todoDateTime.slice(0, 10); 

            // console.log(now);
            // console.log(years);
            // console.log(todo.todoDateTime);
            // console.log(today);
            // console.log(specifyDay);
